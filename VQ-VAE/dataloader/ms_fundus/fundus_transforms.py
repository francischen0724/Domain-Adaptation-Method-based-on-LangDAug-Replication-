...
# class Normalize_tf_normal(object):
#     """Normalize a tensor image with mean and standard deviation.
#     Args:
#         mean (tuple): means for each channel.
#         std (tuple): standard deviations for each channel.
...
def keep_fields(orig, new):
    """确保透传 Dataset 附加的字段"""
    if 'label_path' in orig:
        new['label_path'] = orig['label_path']
    if 'img_name' in orig:
        new['img_name'] = orig['img_name']
    if 'dc' in orig:
        new['dc'] = orig['dc']
    return new
...
class add_salt_pepper_noise():
    def __call__(self, sample):
        ...
        if seed > 0.75:
            ...
            X_imgs_copy[coords[0], coords[1], :] = 0
        sample['image'] = X_imgs_copy
        return keep_fields(sample, sample)
...
class elastic_transform():
    def __call__(self, sample):
        ...
        if seed > 0.5:
            ...
            if label is not None:
                transformed_label = transformed_label.astype(np.uint8)
            return keep_fields(sample, {'image': Image.fromarray(transformed_image), 'label': transformed_label})
        return keep_fields(sample, sample)
...
class RandomCrop(object):
    ...
    def __call__(self, sample):
        ...
        th, tw = self.size
        if w == tw and h == th:
            return keep_fields(sample, {'image': img, 'label': mask})
        x1 = random.randint(0, w - tw)
        y1 = random.randint(0, h - th)
        img = img.crop((x1, y1, x1 + tw, y1 + th))
        mask = mask.crop((x1, y1, x1 + tw, y1 + th))
...
class Normalize_tf_normal(object):
    ...
    def __call__(self, sample):
        img = np.array(sample['image']).astype(np.float32) / 127.5 - 1.0
        __mask = np.array(sample['label']).astype(np.uint8)
        ...
        mask = np.expand_dims(__mask, axis=-1)
        return keep_fields(sample, {'image': img, 'label': mask})
...
class ToTensor(object):
    def __call__(self, sample):
        img = np.array(sample['image']).astype(np.float32).transpose((2, 0, 1))
        label = np.array(sample['label']).astype(np.uint8)
        if label.ndim == 2:
            label = label[None, ...]
        else:
        ...
        return keep_fields(sample, {'image': img, 'label': label})
